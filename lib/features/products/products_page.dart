...
class _ProductsPageState extends ConsumerState<ProductsPage> {
                              ...
                              children: [
                                ...
                                for (final subgroup in group.subgroups) ...[
                                  Padding(
                                    ...
                                    child: Text(
                                      '${subgroup.title} (${subgroup.items.length})',
                                      style: Theme.of(context)
                                          .textTheme
                                          .titleMedium,
                                    ),
                                  ...
                                  SingleChildScrollView(
                                    ...
                                    child: LayoutBuilder(
                                      builder: (context, constraints) {
                                        ...
                                        final tableWidth = horizontalMargin * 2 +
                                            codeWidth +
                                            nameWidth +
                                            priceWidth +
                                            autoorderWidth +
  ...
  List<_ProductCategoryGroup> _buildGroups(
    ...
    final categoryLookup = {for (final category in categories) category.id: category};
    ...
    return [
      for (final key in keys)
        _ProductCategoryGroup(
          key,
          _buildSubgroups(key, grouped[key]!),
        ),
    ];
  }
  ...
  List<_ProductSubgroup> _buildSubgroups(
    ...
    if (categoryTitle == '营养补充品' &&
        subgroupKeys.contains('基础营养素')) {
      subgroupKeys
        ..remove('基础营养素')
        ..insert(0, '基础营养素');
    }
